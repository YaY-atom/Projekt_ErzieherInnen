  ...
  create_master_erzieherinnen <- function(parquet_dir, output_dir) {
    ...
    for (pf in parquet_files) {
      ...
      if (str_detect(file_name, "kldb")) {
        ...
        if("id" %in% names(df)) df <- dplyr::rename(df, posting_id = id)
        ...
        if("concept" %in% names(df)) df <- dplyr::rename(df, skills = concept)
      } else if (str_detect(file_name, "transversal")) {
        # Nur umbenennen, wenn Zielspalte noch nicht existiert
        ...
        if("concept" %in% names(df)) df <- dplyr::rename(df, benefits = concept)
      } else {
        next
      }
