...
## chunkbasiertes verarbeiten aller Dateien:
...
local({
  ...
  # --- Definiere gewünschte Spalten und ihre R-Typen ---
  relevant_cols <- c(
  ...
  relevant_cols_types <- list(
    posting_id = "character",
    kldb_id = "character",
    taetigkeitsniveau = "character",
    year = "integer",
    region = "character",
    job_title = "character",
    language = "character",
    advertiser_type_id = "character",
    contract_type_id = "character",
    remote_category = "character",
    skills = "character",
    transversal_skills = "character",
    benefits = "character",
    published_at = "POSIXct",
    source_type_id = "character"
  )
  ...
  create_master_erzieherinnen <- function(parquet_dir, output_dir) {
    ...
    for (pf in parquet_files) {
      ...
      df <- tryCatch(arrow::read_parquet(pf) %>% as_tibble(), error = function(e) NULL)
      if (is.null(df) || nrow(df) == 0) next

      # --- Datei-spezifische Anpassungen ---
      if (str_detect(file_name, "kldb")) {
        df <- df %>%
          filter(str_starts(kldb_id, "8311")) %>%
          mutate(year = as.integer(year),
                 taetigkeitsniveau = extract_tn(kldb_id))
      } else if (str_detect(file_name, "published_at_source_type_location") |
                 str_detect(file_name, "sprache_advertiser_contract") |
                 str_detect(file_name, "Job-Title")) {
        if("id" %in% names(df)) df <- rename(df, posting_id = id)
      } else if (str_detect(file_name, "Verbis_Fachkompetenzen_Erzieher")) {
        if("concept" %in% names(df)) df <- rename(df, skills = concept)
      } else if (str_detect(file_name, "transversal")) {
        if("posting_id" %in% names(df) && !"transversal_skills" %in% names(df)) {
          df <- rename(df, transversal_skills = posting_id)
        }
      } else if (str_detect(file_name, "Benefits") | str_detect(file_name, "family_compatibility") | str_detect(file_name, "TSC")) {
        if("concept" %in% names(df)) df <- rename(df, benefits = concept)
      ...
      # --- Fehlende Spalten hinzufügen und Typen forcieren ---
      for (col in relevant_cols) {
        if (!col %in% names(df)) {
          df[[col]] <- switch(relevant_cols_types[[col]],
                              "character" = as.character(NA),
                              "integer" = as.integer(NA),
                              "POSIXct" = as.POSIXct(NA, origin = "1970-01-01"))
        } else {
          df[[col]] <- switch(relevant_cols_types[[col]],
                              "character" = as.character(df[[col]]),
                              "integer" = as.integer(df[[col]]),
                              "POSIXct" = as.POSIXct(df[[col]], origin = "1970-01-01"),
                              df[[col]])
        }
      }

      df <- df %>% select(all_of(relevant_cols))
    ...
    readr::write_csv(master_log, file.path(dirname(output_dir), "master_erzieherinnen_log.csv"))
  ...
  ds <- arrow::open_dataset(master_dir, format = "parquet")
